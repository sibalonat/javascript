...
export function createBoard(boardSize, numberOfMines) {
    ...
    for (let x = 0; x < boardSize; x++) {
        ...
        for (let y = 0; y < boardSize; y++) {
            ...
            element.setAttribute('class', 'bordered'),
            ...
            const tile = {
                ...
                x,
                y,
                mine: minePosition.some(positionMatch.bind(null, {
                    x,
                    y
                })),
                get status() {
...
export function revealTile(tile, board) {
    ...
    const adjentTiles = nearbyTiles(board, tile);
    const mines = adjentTiles.filter(t => t.mine);
    if (mines.length === 0) {
        adjentTiles.forEach(revealTile.bind(null, board))
    } else [
